...
control process_int_report (
    ...
    action add_report_fixed_header(switch_id_t switch_id, seq_t seq) {
        ...
        hdr.report_fixed_header.sw_id = switch_id;

        hdr.report_fixed_header.seq_no = seq;
        
        hdr.report_fixed_header.ingress_tstamp = (bit<32>) standard_metadata.ingress_global_timestamp;
        hdr.report_fixed_header.engress_tstamp = (bit<32>) standard_metadata.egress_global_timestamp;

    }

    action do_report_encapsulation(mac_t src_mac, mac_t mon_mac, ip_address_t src_ip,
                        ip_address_t mon_ip, l4_port_t mon_port, switch_id_t switch_id, seq_t seq) {
        ...
        add_report_fixed_header(switch_id, seq);
    ...
    table tb_generate_report {
        key = {
            IS_I2E_CLONE(standard_metadata)  : exact @name("report_is_true");
